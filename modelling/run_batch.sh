...
network='resnet50'
seed=0
for num_training_images_per_class in 5 10 15 20
do
  mkdir -p "${PROJECT_ROOT}"/log/n${num_training_images_per_class}

  # model specific parameters
  enable_attn_loss=True
  for attn_loss_scalar in 0.1 0.01 0.001
    do
      identifier=cub_attention_${network}_lr${lr}_b${batch_size}_ls${attn_loss_scalar}_s${seed}
      echo ${identifier}
